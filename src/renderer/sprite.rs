...
use crate::renderer::texture::ArrayTexture;
use crate::renderer::TEXTURE_ARRAY_SIZE;
...
impl Sprite {
    pub fn new(
        ...
        let textures: Vec<ArrayTexture> = frames
            .iter()
            .map(|image| ArrayTexture::new(&device, &queue, image))
            .collect();
